...
# ================= ADYEN (from bash script) =================
async def adyen_check(card, month, year, cvv):
    try:
        session = requests.Session()
        # Step 1: get checkout attempt id
        ...
        try:
            ...
            checkout_id = analytics_json.get('checkoutAttemptId')
        except:
            checkout_id = None
        if not checkout_id:
        ...
        # Step 2: get access token from picsart
        ...
        # Step 3: encrypt card using external API
        ...
        enc_resp = session.post(encrypt_url, json=encrypt_payload)
        ...
        payment_headers = {
            'authority': 'api.picsart.com',
            'accept': '*/*',
            'accept-language': 'en-US,en;q=0.9',
            'content-type': 'application/json',
            'deviceid': 'a.s.mqndoz2c.7757675f-f50e-440b-81ab-bffd1ce7890a',
            'origin': 'https://picsart.com',
            'platform': 'website',
        ...
        pay_resp = session.post('https://api.picsart.com/shop/subscription/adyen/purchase', headers=payment_headers, json=payment_payload, timeout=30)
        if pay_resp.status_code != 200:
            return ("❌ ERROR", f"Payment error: {pay_resp.status_code}", None)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Try to send local dragon.jpg (if exists)
    try:
...
async def account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• ID: {user.id}\n"
        f"• Plan: {plan}\n"
        f"• Mass Limit: {mass_limit}\n"
        f"• Private Access: {private}\n"
        f"• Plan Expires: {expiry}\n"
    )
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'main_gates':
        ...
        await query.edit_message_text("🔽 **Select a gateway**", reply_markup=InlineKeyboardMarkup(keyboard))
    elif data == 'main_account':
        user = update.effective_user
        plan = "Pro"
        text = f"👤 *User Info*\n• Name: {user.first_name}\n• ID: {user.id}\n• Plan: {plan}\n• Mass Limit: 5000\n• Private Access: On\n• Plan Expires: 2126-05-06"
        await query.edit_message_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ BACK", callback_data='main_back')]]))
    elif data == 'main_tools':
        await query.edit_message_text("🛠 Tools coming soon...", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ BACK", callback_data='main_back')]]))
    elif data == 'main_close':
        await query.edit_message_text("👋 Closed. Use /start to reopen.")
    elif data == 'main_back':
...
async def start_again(query):
    caption = "🐉 **Welcome Lenin** » ...\nThis bot promises you fast and safe checkups..."
    keyboard = [
        [InlineKeyboardButton("🚪 GATES", callback_data='main_gates')],
        [InlineKeyboardButton("👤 ACCOUNT", callback_data='main_account')],
        [InlineKeyboardButton("🛠 TOOLS", callback_data='main_tools')],
...
async def gateway_command(update: Update, context: ContextTypes.DEFAULT_TYPE, gateway_name: str):
    ...
    if gateway_name == 'shopify':
        ...
        status, msg, live = await authorize_check(card, month, year, cvv)  # reuse
